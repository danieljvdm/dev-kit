...
Propose: _“I recommend Error Monitoring + Tracing + Session Replay first.
Want me to also enable Profiling, Logs, and User Feedback?”_
...
export async function loader() {
  return Sentry.startSpan({ op: "test", name: "My First Test Transaction" }, () => {
    throw new Error("My first Sentry error!");
  });
}
```
...
| Non-framework app configured with `@sentry/react-router` | Switch to [`react`](../react/index.md) + `@sentry/react` for v5/v6/v7 non-framework routes |
