...
authenticate. Because `flush()` only uploads _already-active_ recording data, guard on
...
// After authentication, always record a replay for internal employees
if (loggedInUser.isEmployee) {
  const replay = Sentry.getReplay();

  if (replay.getReplayId()) {
    // A replay is already active (session or buffer mode):
    // flush uploads the pending data and continues recording.
    replay.flush();
  } else {
    // Nothing recording yet: start a full session-mode recording.
    replay.start();
  }
}
```
