...
3. Forwards logs as **native Sentry structured logs** (SDK ≥ 6.0.0)
...
| `AddLogEntryFilter(...)` | method                 | —             | Register a custom `ILogEntryFilter` for pre-processing (the underlying `Filters` array is internal) |
| `TagFilters`             | `IList<StringOrRegex>` | —             | Inherited from core `SentryOptions`; drops tags whose key matches by substring or regex             |
...
LogManager.Configuration.AddSentry(options =>
{
    ...
    options.IncludeEventPropertiesAsTags = true;
});
...
| `IncludeEventPropertiesAsTags`  | `false` | Forward NLog properties as Sentry tags          |
