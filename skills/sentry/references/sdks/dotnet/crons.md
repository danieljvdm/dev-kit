...
| `MaxRuntime`               | `TimeSpan` | Maximum allowed runtime before a timeout alert          |
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            var checkInId = SentrySdk.CaptureCheckIn(
                ...
                {
                    o.Interval("0 2 * * *"); // 2 AM daily
                    o.CheckInMargin = TimeSpan.FromMinutes(15);
                    o.MaxRuntime = TimeSpan.FromMinutes(60);
                    o.TimeZone = "UTC";
    ...
    private void RunJob(object? state)
    {
        var checkInId = SentrySdk.CaptureCheckIn(
            ...
            configureMonitorOptions: o =>
            {
                ...
                o.CheckInMargin = TimeSpan.FromMinutes(5);
                o.MaxRuntime = TimeSpan.FromMinutes(30);
                o.TimeZone = "UTC";
            }
        );
...
public class MyJob
{
    [SentryMonitorSlug("my-hangfire-job")]
    public void Execute() { /* ... */ }
}
```
...
public class MyQuartzJob : IJob
{
    public async Task Execute(IJobExecutionContext context)
    {
        ...
        var checkInId = SentrySdk.CaptureCheckIn(slug, CheckInStatus.InProgress,
            configureMonitorOptions: o =>
            {
                o.Interval("0 */6 * * *"); // every 6 hours
                o.CheckInMargin = TimeSpan.FromMinutes(10);
                o.MaxRuntime = TimeSpan.FromMinutes(60);
                o.TimeZone = "UTC";
            }
        );
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            var checkInId = SentrySdk.CaptureCheckIn(
                ...
                configureMonitorOptions: o =>
                {
                    ...
                    o.CheckInMargin = TimeSpan.FromMinutes(2);
                    o.MaxRuntime = TimeSpan.FromMinutes(10);
                    o.TimeZone = "UTC";
                }
            );
...
| Crontab schedule (`Interval(crontab)`)                  | **4.7.0**          |
| Interval schedule (`SentryMonitorInterval`)             | **4.7.0**          |
