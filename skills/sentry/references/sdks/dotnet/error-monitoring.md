...
**BreadcrumbLevel values:** `Debug`, `Info` (default), `Warning`, `Error`, `Fatal`
...
| `SampleRate`            | `float?`                     | `null` (sends 100%)    | Error event sampling rate (0–1)                                   |
| `TracesSampleRate`      | `double?`                    | `null` (tracing off)   | Transaction sampling rate (0–1)                                   |
...
| `DefaultTags`           | `Dictionary<string, string>` | `{}`                   | Tags added to every event                                         |
