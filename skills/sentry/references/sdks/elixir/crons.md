...
defmodule MyApp.Crons do
  ...
  def monitor(slug, fun, monitor_config \\ []) do
    # capture_check_in/1 returns {:ok, id}, :ignored (no DSN / filtered env),
    # or {:error, _} (HTTP failure). Match all three so a non-:ok result never
    # raises a MatchError and crashes the job.
    check_in_id =
