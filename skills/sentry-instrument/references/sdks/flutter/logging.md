...
Sentry.logger.trace('Starting checkout flow', attributes: {'step': SentryAttribute.string('init')});

// Development diagnostics
Sentry.logger.debug('Cache lookup', attributes: {
  'key': SentryAttribute.string('user:123'),
  'hit': SentryAttribute.bool(false),
});

// Normal operations and business milestones
Sentry.logger.info('Order created', attributes: {
  'orderId': SentryAttribute.string('order_456'),
  'total': SentryAttribute.double(99.99),
});

// Degraded state, approaching limits
Sentry.logger.warn('Rate limit approaching', attributes: {
  'endpoint': SentryAttribute.string('/api/search/'),
  'current': SentryAttribute.int(95),
  'max': SentryAttribute.int(100),
});

// Failures requiring attention
Sentry.logger.error('Payment failed', attributes: {
  'reason': SentryAttribute.string('card_declined'),
  'userId': SentryAttribute.string('u_1'),
});

// Critical failures — app or subsystem is down
Sentry.logger.fatal('Database unavailable', attributes: {'host': SentryAttribute.string('db-primary')});
```
...
class PaymentService {
  Future<void> processPayment(String orderId, double amount) async {
    _logger.info('Processing payment for order $orderId (amount: $amount)');

    try {
      final result = await _chargeCard(amount);
      _logger.info(
          'Payment succeeded for order $orderId (transaction: ${result.transactionId})');
    } catch (e, stackTrace) {
...
Sentry.logger.info('Checkout completed', attributes: {
  'orderId': SentryAttribute.string(order.id),
  'userId': SentryAttribute.string(user.id),
  'cartValue': SentryAttribute.double(cart.total),
  'itemCount': SentryAttribute.int(cart.items.length),
  'paymentMethod': SentryAttribute.string('stripe'),
  'durationMs': SentryAttribute.int(
      DateTime.now().difference(startTime).inMilliseconds),
});

Sentry.logger.error('Navigation failed', attributes: {
  'fromRoute': SentryAttribute.string('/home'),
  'toRoute': SentryAttribute.string('/profile'),
  'errorCode': SentryAttribute.string(err.code),
  'retryable': SentryAttribute.bool(true),
});
```
...
Sentry.logger.info('Validating cart', attributes: {'cartId': SentryAttribute.string(cart.id)});
...
Future<void> processOrder(String orderId) async {
  ...
  try {
    Sentry.logger.info('Validating cart', attributes: {'orderId': SentryAttribute.string(orderId)});
    await validateCart(orderId);

    Sentry.logger.info('Charging payment', attributes: {'orderId': SentryAttribute.string(orderId)});
    await chargePayment(orderId);

    Sentry.logger.info('Confirming order', attributes: {'orderId': SentryAttribute.string(orderId)});
