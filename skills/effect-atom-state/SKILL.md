...
description: Implement React queries, mutations, shared state, and workflows with Effect Atom.
---
...
- API clients, queries, or mutations: [client state](references/effect-atom-client.md).
- Multi-step actions, optimistic updates, or Promise dispatch:
  [workflows](references/effect-atom-workflows.md).
- Registry scope, atom identity, retention, freshness, polling, or cancellation:
  [lifecycle](references/effect-atom-lifecycle.md).
...
- A regression test allowed by repository policy or explicitly requested by the
  user: [testing](references/effect-atom-testing.md). Read only the relevant cases.

## Ownership constraints

- Derive clients from the shared HTTP contract. Change contracts and server
  handlers with `build-effect-apis` when that work is in scope.
